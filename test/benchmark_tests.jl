...
setups = [
    Dict(:alg => DP5())
    Dict(:alg => Tsit5(), :abstols => 1 ./ 10 .^ (4:7), :reltols => 1 ./ 10 .^ (1:4))
]
...
sol = solve(
    prob,
    MethodOfSteps(Vern9(), fpsolve = NLFunctional(; max_iter = 1000));
    reltol = 1e-8,
    abstol = 1e-8,
)
test_sol = TestSolution(sol)
...
setups = [Dict(:alg => RadauIIA5()), Dict(:alg => RosShamp4())]
