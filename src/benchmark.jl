...
function Shootout(prob, setups; appxsol = nothing, names = nothing, error_estimate = :final,
    ...
    let _prob = _prob
        ...
        for i in 1:N
            if dts === nothing
                ...
                println("Yes, it reach here")
                println("THe problem is: ", _prob)
                println("The algorithm is: ", alg)
                println("dt is dts: ", dts[i])
...
function WorkPrecision(prob::NonlinearProblem,
    alg,
    abstols,
    reltols,
    dts = nothing;
