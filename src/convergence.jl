...
function test_convergence(
    ...
    save_start = true,
    save_everystep = true,
    timeseries_errors = save_everystep,
    adaptive = false,
...
function analyticless_test_convergence(
    ...
    for j = 1:trajectories
        ...
        if use_noise_grid
            if prob.noise_rate_prototype === nothing
                ...
                brownian_values = cumsum(
                    [
                        ...
                        [
                            sqrt(test_dt) * randn(size(prob.noise_rate_prototype, 2))
                            for i = 1:length(t)-1
                        ]
                    ],
                )
            ...
            true_sol =
                solve(prob, alg; adaptive = adaptive, dt = test_dt, save_noise = true)
    ...
    _solutions = [EnsembleSolution(tmp_solutions[:, i], 0.0, true) for i = 1:length(dts)]
    solutions = [
        DiffEqBase.calculate_ensemble_errors(
            sim;
            weak_timeseries_errors = weak_timeseries_errors,
