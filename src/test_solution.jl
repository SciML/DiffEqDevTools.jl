...
TestSolution(interp::DESolution) =
    TestSolution{Nothing,0,true,Nothing,Nothing,typeof(interp)}(
        nothing,
        nothing,
        interp,
        true,
...
function appxtrue(sol::AbstractODESolution, sol2::TestSolution)
    ...
    if _sol.dense
        ...
        errors[:l2] = sqrt(
            recursive_mean(vecvecapply((x) -> float(x) .^ 2, sol - timeseries_analytic)),
        )
        densetimes = collect(range(sol.t[1], stop = sol.t[end], length = 100))
        interp_u = sol(densetimes)
        interp_analytic = _sol(densetimes)
        interp_errors = Dict(
            :L∞ => maximum(vecvecapply((x) -> abs.(x), interp_u - interp_analytic)),
            :L2 => sqrt(
                recursive_mean(
...
function appxtrue(
    ...
    sol2::AbstractODESolution;
    timeseries_errors = sol2.dense,
    dense_errors = sol2.dense,
)
...
function appxtrue(sim::EnsembleSolution, appx_setup; kwargs...)
    ...
    for i in eachindex(sim)
        prob = sim[i].prob
        prob2 =
            SDEProblem(prob.f, prob.g, prob.u0, prob.tspan, noise = NoiseWrapper(sim[i].W))
